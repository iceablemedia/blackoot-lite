...
function blackoot_settings_admin_scripts() {
	$blackoot_template_directory_uri = get_template_directory_uri();
	wp_register_style( 'blackoot_admin_css', $blackoot_template_directory_uri .'/functions/icefit-options/style.css');
	wp_enqueue_style( 'blackoot_admin_css' );
	wp_register_script( 'blackoot_admin_js', $blackoot_template_directory_uri . '/functions/icefit-options/functions.js', array( 'jquery' ), false, true );
	$blackoot_translation_array = array(
		'settings_saved' => __( 'Settings saved.', 'blackoot' ),
		'reset_confirm' => __( 'Are you sure you want to reset ALL settings for this theme to default values?', 'blackoot' ),
	);
	wp_localize_script( 'blackoot_admin_js', 'blackoot_js_strings', $blackoot_translation_array );
	wp_enqueue_script( 'blackoot_admin_js' );
}
...
function blackoot_settings_machine($options) {
	...
	foreach ($options as $arg) {
		...
		elseif ( $arg['type'] == "image" )
		{
			...
			$output .= '<input class="blackoot_remove_button" name="'. $arg['id'] .'_remove" id="'. $arg['id'] .'_remove" type="button" value="' . __('Remove', 'blackoot') . '"><br />'."\n";
			$output .= '<img class="blackoot_image_preview" id="'. $arg['id'] .'_preview" src="'.esc_url($val) .'"><br class="clear">'."\n";
		}
		elseif ( $arg['type'] == "gopro" )
		{
			$output .= '<h3>'. $arg['name'] .'</h3>'."\n";
			$output .= sprintf( __('<p>Unleash the full potential of Blackoot by upgrading to Blackoot Pro!</p>
...
<a href="%s" class="button-primary" target="_blank">Learn More and Upgrade Now!</a>', 'blackoot'),
	'http://www.iceablethemes.com/shop/blackoot-pro/?utm_source=lite_theme&utm_medium=go_pro&utm_campaign=blackoot_lite');
		...
		elseif ( $arg['type'] == "support_feedback" )
		{
			...
			$output .= '<p><a href="http://wordpress.org/support/view/theme-reviews/blackoot-lite" target="_blank" class="button-primary">'.__('Rate and review Blackoot Lite at WordPress.org','blackoot').'</a></p>';

			$output .= '<h3>'.__('Get social!', 'blackoot').'</h3>'."\n";
			$output .= '<p>'.__('Follow and like IceableThemes!','blackoot').'</p>';
...
function blackoot_settings_page(){
	...
	if (isset( $_POST['reset-no-js'] ) && $_POST['reset-no-js']) {
		...
		echo '<div class="updated fade"><p>'.__('Settings were reset to default.', 'blackoot').'</p></div>';
	}
	
	if (isset( $_POST['save-no-js'] ) && $_POST['save-no-js']) {
		blackoot_settings_save_nojs();
		echo '<div class="updated fade"><p>'.__('Settings updated.', 'blackoot').'</p></div>';
	}
	...
	Make sure everything is up to date or try to deactivate some plugins.', 'blackoot'); ?></p></div></noscript><?php

	/* The automatically generated fallback menu is not responsive.
	 * Add a notice to warn users who did not set a primary menu. */
	if ( !has_nav_menu( 'primary' ) ):
	    echo '<div class="update-nag"><p>' . sprintf( __('<strong>Blackoot Lite Notice:</strong> you have not set your primary menu yet, and your site is currently using a fallback menu which is not responsive. Please take a minute to <a href="%s">set your menu now</a>!','blackoot'), admin_url('nav-menus.php') ) . '</p></div>';
    endif;
		...
		<form enctype="multipart/form-data" id="icefitform" method="POST">
			...
			<div id="icefit-admin-panel-footer">
				<div id="icefit-admin-panel-footer-submit">
					...
					<input type="submit" value="<?php _e('Save all Changes','blackoot'); ?>" class="button-primary" id="submit-button" />
					<!-- No-JS Fallback buttons -->
					<noscript>
					<input type="submit" class="button" id="icefit-reset-button-no-js" name="reset-no-js" value="Reset Options" />
					<input type="submit" class="button-primary" id="submit-button-no-js" name="save-no-js" value="<?php _e('Save all Changes','blackoot'); ?>" />
					</noscript>
